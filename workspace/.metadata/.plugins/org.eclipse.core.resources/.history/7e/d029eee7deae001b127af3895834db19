...
<script>
	function checkPostPassword(postPasswordParam, postNumberParam){
		...
		var postNumber = postNumberParam;
		urlPostDetails = "/postDetails?postNumber=" + postNumber;
		urlGoBoard = "redirect:/board";
	...
	function goDetails(){
		window.location.href=urlPostDetails;
	}
	
	function goBoard(){
		window.location.href=urlGoBoard;
	}
</script>
</head>
				...
				<c:forEach items="${post}" var="post">
					<tr>
						...
						<c:if test="${postPassword ne null}">
							<td><a
								href="#" onclick="checkPostPassword(${post.postPassword}, ${post.postNumber});">${post.postTitle}</a></td>
						</c:if>
						<c:if test="${postPassword eq null}">
							<td><a href="/postDetails?postNumber=${post.postNumber}">${post.postTitle}</a></td>
						</c:if>
